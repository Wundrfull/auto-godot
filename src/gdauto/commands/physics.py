...
from gdauto.formats.values import Vector2
...
def add_body(
    ...
    Examples:
      ...
      gdauto physics add-body --scene scenes/main.tscn --name Player --type character --shape rectangle --size 16,32

      gdauto physics add-body --scene scenes/main.tscn --name Coin --type area --shape circle --size 12

      gdauto physics add-body --scene scenes/game.tscn --name Wall --type static --shape rectangle --size 64,16
    """
    try:
...
def add_shape(
    ...
    Examples:
      ...
      gdauto physics add-shape --scene scenes/main.tscn --parent Player --shape circle --size 16

      gdauto physics add-shape --scene scenes/main.tscn --parent Wall --shape rectangle --size 64,16 --name HitBox
    """
    try:
        path = Path(scene_path)
        text = path.read_text(encoding="utf-8")
